...
# merge all preprocessing imputing, normalization and encoding
def manage_data(train_data: pd.DataFrame
)-> Tuple[pd.DataFrame, pd.Series, pd.DataFrame, pd.Series,pd.DataFrame, pd.Series]:
    # first cleaning
    # call_function_to_cleaning()
    # save to pickle after cleaning

    # second split in sets
    app_train_set, app_val_set, app_test_set = get_feature_in_set(train_data)
    
    # third apply preprocess in sets
    train_prep, val_prep, test_prep = preprocess_data(app_train_set,app_val_set,app_test_set)
    
    # fourth split features and target 
    X_train, y_train, X_val, y_val, X_test,y_test = get_feature_target(train_prep, val_prep, test_prep)
    ...
    # save variables dictionary after of preprocessing
    with open('features_target.pkl', 'wb') as file:
        pickle.dump(set_features_target, file)