...
def get_datasets() -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    if not os.path.exists(config.DATASET_TRAIN):
        s3.download_file(
            ...
            f"{config.DATASET_ROOT_PATH}/train_data.csv",
        )
    ...
    if not os.path.exists(config.DATASET_TEST):
        s3.download_file(
            ...
            f"{config.DATASET_ROOT_PATH}/test_data.csv",
        )
    ...
    if not os.path.exists(config.DATASET_DESCRIPTION):
        s3.download_file(
            ...
            f"{config.DATASET_ROOT_PATH}/description.xls",
        )

    # create datasets
    df_excel = pd.read_excel(config.DATASET_DESCRIPTION, index_col=0, engine="xlrd")
    app_train = pd.read_csv(
        config.DATASET_TRAIN, delimiter="\t", encoding="latin1", header=None
    )
    app_test = pd.read_csv(
        config.DATASET_TEST, delimiter="\t", encoding="latin1", header=None
    )
...
def get_feature(df, target_col="TARGET_LABEL_BAD=1"):
    ...
    X_temp, X_test, y_temp, y_test = train_test_split(
        df_train, target, test_size=0.2, random_state=42
    )
